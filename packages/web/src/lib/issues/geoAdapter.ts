 ...
 *
 * Every action set ends in `dismiss`: a suggestion you can't fix and can't hide
 * nags forever, so Dismiss is offered on every card in every panel. The rail
 * hides a dismissed card behind its "Show dismissed" toggle.
 */
...
const FINDING_FIX: Record<string, ActionSpec> = {
  answer_first: { nature: "fix", actions: ["ai_fix", "manual_fix", "highlight", "dismiss"] },
  question_heading: { nature: "fix", actions: ["ai_fix", "manual_fix", "highlight", "dismiss"] },
  bullets: { nature: "fix", actions: ["ai_fix", "manual_fix", "highlight", "dismiss"] },
  self_contained: { nature: "fix", actions: ["ai_fix", "manual_fix", "highlight", "dismiss"] },
  dedupe_opening: { nature: "fix", actions: ["dedupe", "highlight", "dismiss"] },
  comparison_table: { nature: "add", actions: ["generate", "write_own", "dismiss"] },
  cite_reference: { nature: "fix", actions: ["cite_source", "highlight", "dismiss"] },
  quote_reference: { nature: "fix", actions: ["quote_source", "highlight", "dismiss"] },
  alt_text: { nature: "fix", actions: ["generate", "write_own", "highlight", "dismiss"] },
};

/** Lever-level fix (used when a finding carries no per-finding fix). */
const LEVER_FIX: Record<string, ActionSpec> = {
  faq: { nature: "add", actions: ["generate", "write_own", "dismiss"] },
  takeaways: { nature: "add", actions: ["generate", "write_own", "dismiss"] },
  definitional: { nature: "add", actions: ["generate", "write_own", "dismiss"] },
  definitional_improve: { nature: "fix", actions: ["ai_fix", "manual_fix", "dismiss"] },
  comparison_table: { nature: "add", actions: ["generate", "write_own", "dismiss"] },
  ...
  answer_first: { nature: "fix", actions: ["ai_fix", "manual_fix", "dismiss"] },
};
...
function specFor(lever: GeoLever, findingFix: string): ActionSpec {
  ...
  if (lever.key === "brand_explicit") {
    return { nature: "fix", actions: ["ai_fix", "manual_fix", "highlight", "dismiss"] };
  }
  if (lever.key === "factual_density") {
    ...
    return { nature: "fix", actions: ["add_fact", "ai_fix", "highlight", "dismiss"] };
  }
