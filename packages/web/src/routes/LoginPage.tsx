const ERROR_MESSAGES: Record<string, string> = {
  not_allowed:
    "That GitHub account isn't on the allowlist. Ask the site administrator to add your GitHub username, then try again.",
  bad_state: "Your sign-in link expired. Please try again.",
  oauth_denied: "GitHub sign-in was cancelled.",
  github_failed: "GitHub sign-in failed — please try again.",
  github_not_configured:
    "Sign-in isn't configured on this server yet. Contact the administrator.",
};
...
export function LoginPage(): JSX.Element {
  ...
  const errorMessage = errorCode
    ...
    : null;
  // Retrying can't help when the server has no GitHub OAuth configured — hide
  // the button so it's not a dead-end loop.
  const canSignIn = errorCode !== "github_not_configured";
