# Pencraft v1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build Pencraft v1 — a local-first long-form drafting tool that consumes myvoice style packs through a 3-stage (idea → outline → sections) wizard with persistent drafts.

**Architecture:** Fresh repo at `/Users/dbbaskette/Projects/Pencraft`. FastAPI backend + React/Vite frontend mirroring myvoice's two-package layout. Imports `myvoice` as a Python library for pack discovery, prompt composition, and lint. LLM and jobs/SSE machinery copied from myvoice (consolidate later). Drafts persist as JSON to `~/.pencraft/drafts/<id>/`.
...
**Spec:** [`docs/superpowers/specs/2026-05-26-pencraft-v1-design.md`](../specs/2026-05-26-pencraft-v1-design.md)

**Branch:** `pencraft-v1` (already checked out). 11 commits, one per task. Open one PR at the end.
...
When copying, change `myvoice` → `pencraft` in imports and module docstrings. Otherwise the code is identical.
...
- `packages/api/pencraft/__init__.py`, `cli.py`, `server.py`
...
[tool.hatch.build]
artifacts = ["packages/api/pencraft/static/**/*"]

[tool.hatch.build.targets.wheel]
packages = ["packages/api/pencraft"]
...
- [ ] **Step 1.7: packages/api/pencraft/__init__.py**
...
- [ ] **Step 1.8: packages/api/pencraft/cli.py**
...
- [ ] **Step 1.9: packages/api/pencraft/server.py (minimal stub)**
...
- pencraft CLI with `serve` and `version` (defaults to port 7880)
...
Copy myvoice's `packages/api/myvoice/llm/` directory into `packages/api/pencraft/llm/`. Change all `myvoice` references to `pencraft`. Same code, same tests, same behavior.

**Files:**
- Create: `packages/api/pencraft/llm/{__init__.py, base.py, exceptions.py, anthropic.py, openai.py, google.py, rates.yaml, rates.py, cost.py, registry.py}`
- Create: `packages/api/tests/llm/{__init__.py, test_anthropic.py, test_openai.py, test_google.py, test_cost.py, test_registry.py}`
- Create: `packages/api/pencraft/test_helpers/{__init__.py, mock_provider.py}`

- [ ] **Step 2.1: Copy llm directory**

```bash
cp -R /Users/dbbaskette/Projects/myvoice/packages/api/myvoice/llm \
      /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/llm

# Replace `myvoice` with `pencraft` in imports + docstrings
find /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/llm -name "*.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g; s/import myvoice/import pencraft/g; s/myvoice\.llm/pencraft.llm/g' {} \;
```
...
cp -R /Users/dbbaskette/Projects/myvoice/packages/api/myvoice/test_helpers \
      /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/test_helpers

find /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/test_helpers -name "*.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g; s/MYVOICE_/PENCRAFT_/g' {} \;
```

The mock provider's env vars become `PENCRAFT_TEST_PROVIDER`, `PENCRAFT_MOCK_OUTPUT`, `PENCRAFT_MOCK_OUTPUT_JSON`.

Update `packages/api/pencraft/llm/registry.py` similarly — the env var check inside `get_provider()` becomes `PENCRAFT_TEST_PROVIDER`.

- [ ] **Step 2.3: Copy LLM tests**

```bash
cp -R /Users/dbbaskette/Projects/myvoice/packages/api/tests/llm \
      /Users/dbbaskette/Projects/Pencraft/packages/api/tests/llm

find /Users/dbbaskette/Projects/Pencraft/packages/api/tests/llm -name "*.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g; s/MYVOICE_/PENCRAFT_/g' {} \;
```
...
find /Users/dbbaskette/Projects/Pencraft/packages/api/tests/llm -name "test_*structured.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g' {} \;
```
...
rewritten to pencraft. Same 3 provider adapters (Anthropic via tool-use,
...
PENCRAFT_TEST_PROVIDER=mock + PENCRAFT_MOCK_OUTPUT[_JSON].
...
**Files:**
- Create: `packages/api/pencraft/jobs/{__init__.py, models.py, registry.py, events.py}`
...
cp -R /Users/dbbaskette/Projects/myvoice/packages/api/myvoice/jobs \
      /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/jobs

find /Users/dbbaskette/Projects/Pencraft/packages/api/pencraft/jobs -name "*.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g; s/myvoice\.jobs/pencraft.jobs/g' {} \;
```

- [ ] **Step 3.2: Change JobType enum**

Edit `packages/api/pencraft/jobs/models.py`. Replace the `JobType` enum body:
```python
...
(Pencraft only has these two job types; rewrite and extract are myvoice-specific.)

- [ ] **Step 3.3: Copy jobs tests + adapt**

```bash
cp -R /Users/dbbaskette/Projects/myvoice/packages/api/tests/jobs \
      /Users/dbbaskette/Projects/Pencraft/packages/api/tests/jobs

find /Users/dbbaskette/Projects/Pencraft/packages/api/tests/jobs -name "*.py" -exec \
    sed -i '' 's/from myvoice/from pencraft/g; s/JobType\.REWRITE/JobType.EXPAND/g; s/JobType\.EXTRACT/JobType.REGEN_SECTION/g' {} \;
```
...
replay snapshot. JobType is { EXPAND, REGEN_SECTION } for Pencraft's
...
**Files:**
- Create: `packages/api/pencraft/drafts/{__init__.py, models.py, store.py}`
- Create: `packages/api/pencraft/api/{__init__.py, drafts.py}`
- Modify: `packages/api/pencraft/server.py` (lifespan loads DraftStore, mounts router)
- Create: `packages/api/tests/drafts/{__init__.py, test_store.py}`
- Create: `packages/api/tests/api/{__init__.py, test_drafts_route.py}`

- [ ] **Step 4.1: Models**

`packages/api/pencraft/drafts/__init__.py`:
```python
...
from pencraft.drafts import Draft, DraftStore, IdeaInput
...
`packages/api/pencraft/api/__init__.py`: empty.

`packages/api/pencraft/api/drafts.py`:
```python
...
Replace the contents of `packages/api/pencraft/server.py`:
...
under ~/.pencraft/drafts/<id>/ with atomic JSON writes + always-current
post.md snapshot. Delete soft-moves to ../trash/<ts>-<id>/.

CRUD routes: GET/POST/PUT/DELETE /api/drafts. Server lifespan loads
DraftStore from PENCRAFT_DRAFTS_ROOT or ~/.pencraft/drafts/.
...
**Files:**
- Create: `packages/api/pencraft/api/{packs.py, providers.py}`
- Modify: `packages/api/pencraft/server.py` (mount routers, expose PackStore via state)
- Create: `packages/api/tests/api/{test_packs_route.py, test_providers_route.py}`

- [ ] **Step 5.1: Packs route**

`packages/api/pencraft/api/packs.py`:
```python
...
`packages/api/pencraft/api/providers.py`:
```python
...
Update `packages/api/pencraft/server.py` lifespan to construct a `myvoice.PackStore`:
...
**Files:**
- Create: `packages/api/pencraft/generate/{__init__.py, outline.py}`
- Create: `packages/api/pencraft/generate/prompts/outline.j2`
- Create: `packages/api/pencraft/api/outline.py`
...
`packages/api/pencraft/generate/__init__.py`: empty.

`packages/api/pencraft/generate/prompts/__init__.py`: empty.

`packages/api/pencraft/generate/prompts/outline.j2`:
```
...
`packages/api/pencraft/generate/outline.py`:
```python
...
`packages/api/pencraft/api/outline.py`:
```python
...
**Files:**
- Create: `packages/api/pencraft/generate/section.py`
- Create: `packages/api/pencraft/generate/prompts/section.j2`
- Create: `packages/api/pencraft/api/{expand.py, section.py, jobs.py}`
- Modify: `packages/api/pencraft/server.py` (lifespan: JobRegistry; mount routers)
...
`packages/api/pencraft/generate/prompts/section.j2`:
```
...
`packages/api/pencraft/generate/section.py`:
```python
...
from pencraft.drafts.models import Draft, OutlineSection, Section
...
`packages/api/pencraft/api/expand.py`:
```python
...
`packages/api/pencraft/api/section.py`:
```python
...
from pencraft.api.expand import _read_myvoice_key
from pencraft.drafts import Draft, DraftStore
from pencraft.generate.section import stream_section
from pencraft.jobs.models import JobType
from pencraft.jobs.registry import JobRegistry
...
`packages/api/pencraft/api/jobs.py`:
```python
...
**Files:**
- Create: `packages/api/pencraft/api/{download.py, lint.py, events.py}`
- Modify: `packages/api/pencraft/server.py` (lifespan: EventBus; mount routers)
- Create: `packages/api/tests/api/{test_download_route.py, test_lint_route.py, test_events_route.py}`

- [ ] **Step 8.1: Download route**

`packages/api/pencraft/api/download.py`:
```python
...
`packages/api/pencraft/api/lint.py`:
```python
...
For v1, the global event bus is minimal — emits `draft:*` events. Reuses the EventBus class from myvoice's `watch.py`. Copy the EventBus class only (not the watcher — Pencraft doesn't need file watching in v1).

`packages/api/pencraft/api/events.py`:
```python
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100">
      <header className="border-b border-slate-800 px-6 py-3 flex items-center gap-4">
        <Link to="/" className="text-lg font-semibold">Pencraft</Link>
...
cp /Users/dbbaskette/Projects/myvoice/packages/web/src/components/MarkdownEditor.tsx \
   /Users/dbbaskette/Projects/Pencraft/packages/web/src/components/draft/MarkdownEditor.tsx
```

Edit the file:
1. Remove the `slug` + `path` + auto-save-via-PUT logic — Pencraft saves at the section level, not the file level. Replace with `{ initialMarkdown, onSave }` props.
...
const backendEnv = [
  ...
  `PENCRAFT_MOCK_OUTPUT_JSON='${MOCK_OUTLINE.replace(/'/g, "\\'")}'`,
...
export default defineConfig({
  ...
  webServer: [
    {
      command: `${setup} && ${backendEnv} ${REPO}/.venv/bin/pencraft serve --no-browser --dev --port 7880`,
...
PENCRAFT_TEST_PROVIDER=mock + canned MOCK_OUTPUT_JSON outline.
...
git push -u origin pencraft-v1

gh pr create --title "Pencraft v1: idea → outline → sections" --body "$(cat <<'EOF'
## Summary
...
Reads API keys from ~/.myvoice/config.yaml; no Pencraft Settings page in v1.
Pencraft depends on myvoice as a Python library (PackStore, compose_prompt, lint, validate_pack).
